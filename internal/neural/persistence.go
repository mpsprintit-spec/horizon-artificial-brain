...
func (n *Network) Save(path string) error {
	...
	s.Neurons=make([]*Neuron,0,len(n.neurons)); for _,x:=range n.neurons { y:=*x; s.Neurons=append(s.Neurons,&y) }
	s.Synapses=make([]*Synapse,0,len(n.synapses)); for _,x:=range n.synapses { y:=*x; s.Synapses=append(s.Synapses,&y) }
...
func Load(path string) (*Network,error) {
	...
	for _,x:=range s.Neurons { if x==nil || x.ID==0 || len(n.neurons)>=n.cfg.MaxNeurons{return nil,ErrInvalidConfig}; y:=*x; n.neurons[y.ID]=&y }
	for _,x:=range s.Synapses { if x==nil || x.ID==0{return nil,ErrInvalidConfig}; if _,ok:=n.neurons[x.Pre];!ok{return nil,ErrInvalidID}; if _,ok:=n.neurons[x.Post];!ok{return nil,ErrInvalidID}; y:=*x; y.Weight=clamp(y.Weight,n.cfg.MinWeight,n.cfg.MaxWeight); n.synapses[y.ID]=&y; n.out[y.Pre]=append(n.out[y.Pre],y.ID); n.in[y.Post]=append(n.in[y.Post],y.ID) }
	return n,nil
}
