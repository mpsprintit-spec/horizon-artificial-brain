...
func TestBrainPersistencePreservesDistributedPopulationAndSurfaceAnnotation(t *testing.T) {
	brain := NewBrain()
	first, err := brain.GroundObservation("cup", "camera", "vision", 0.95)
	if err != nil {
		t.Fatal(err)
	}
	dir := t.TempDir()
	...
	if len(restored.ProjectionPopulations) != 1 {
		t.Fatalf("expected one persisted projection population, got %d", len(restored.ProjectionPopulations))
	}
	second, err := restored.GroundObservation("cup", "camera-2", "vision", 0.95)
	if err != nil {
		t.Fatal(err)
	}
	if second.Status != GroundingExisting {
		t.Fatalf("restored population was not reused: status=%s", second.Status)
	}
	if len(second.Population) != len(first.Population) {
		t.Fatalf("restored population size changed: before=%d after=%d", len(first.Population), len(second.Population))
	}
	for i, id := range first.Population {
		if second.Population[i] != id {
			t.Fatalf("restored population changed member %d: before=%d after=%d", i, id, second.Population[i])
		}
	}
	if restored.Fetch("cup") == nil {
		t.Fatal("surface annotation was not restored for adapter compatibility lookup")
	}
}
